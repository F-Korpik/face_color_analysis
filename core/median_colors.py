...
def get_median_colors(image: np.ndarray, landmarks: np.ndarray) -> Tuple[Optional[Tuple[int, int, int]], Optional[np.ndarray]]:
    """
    ...
    mouth_points = landmarks[LIPS_OUTER].astype(np.int32)
    cv2.fillPoly(mouth_mask, [mouth_points], 255)

    # 2. Tworzenie maski obszarów wykluczonych (oczy, brwi, usta)
    exclude_mask_skin = np.zeros((h, w), dtype=np.uint8)
    ...
    for feature_indices in features_to_exclude:
        ...
        cv2.fillPoly(exclude_mask_skin, [feature_points], 255)

    exclude_mask_lips = np.zeros((h, w), dtype=np.uint8)
    feature_points = landmarks[LIPS_INNER].astype(np.int32)
    cv2.fillPoly(exclude_mask_lips, [feature_points], 255)

    # 3. Finalna maska skóry
    skin_mask = cv2.bitwise_and(face_mask, face_mask, mask=cv2.bitwise_not(exclude_mask_skin))
    lips_mask = cv2.bitwise_and(mouth_mask, mouth_mask, mask=cv2.bitwise_not(exclude_mask_lips))

    # 4. Obliczanie MEDIANY
    skin_pixels = image[skin_mask > 0]
    lips_pixels = image[lips_mask > 0]

    if skin_pixels.size == 0:
        return None, skin_mask

    if lips_pixels.size == 0:
        return None, lips_mask

    # Liczymy medianę wzdłuż osi 0 (dla każdego kanału B, G, R osobno)
    skin_median_bgr = np.median(skin_pixels, axis=0)
    lips_median_bgr = np.median(lips_pixels, axis=0)

    # Konwersja na int (np.median zwraca floaty)
    skin_color = (int(skin_median_bgr[0]), int(skin_median_bgr[1]), int(skin_median_bgr[2]))
    lips_color = (int(lips_median_bgr[0]), int(lips_median_bgr[1]), int(lips_median_bgr[2]))

    return skin_color, lips_color, skin_mask, lips_mask